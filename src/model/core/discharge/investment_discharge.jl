...
function investment_discharge!(EP::Model, inputs::Dict, setup::Dict)
	...
	#existing_ngcc = inputs["ngcc_existing"]
	#existing_ngt = inputs["ngt_existing"]
	#retro_ngcc = inputs["ngcc_retro"]
	#retro_ngt = inputs["ngt_retro"]
	...
	#Constraint on retrofit capacity allowed should not be more than retired capacity
	#@constraint(EP, cRetroZCF_CC, sum(EP[:vRETCAP][existing_ngcc]) >= sum(EP[:vCAP][retro_ngcc]))
	#@constraint(EP, cRetroZCF_CT, sum(EP[:vRETCAP][existing_ngt]) >= sum(EP[:vCAP][retro_ngt]))

	#Constraint on retrofit capacity allowed should be subtracted from existing capacity
	##@constraint(EP, c1RetroZCF_CC, sum(EP[:eTotalCap][existing_ngcc]) + sum(EP[:vCAP][retro_ngcc]) <=  35000     )
	#@constraint(EP, c1RetroZCF_CT, sum(EP[:eTotalCap][existing_ngt]) + sum(EP[:vCAP][retro_ngt])  <=   4600    )
