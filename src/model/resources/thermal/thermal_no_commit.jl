...
function thermal_no_commit!(EP::Model, inputs::Dict, setup::Dict)
	...
	nuclear = inputs["nuclear"]              #subset out nuclear power plants
	non_nuke_thermal = setdiff(THERM_NO_COMMIT, nuclear)

	### Expressions ###

	## Power Balance Expressions ##
	@expression(EP, ePowerBalanceThermNoCommit[t=1:T, z=1:Z],
		sum(EP[:vP][y,t] for y in intersect(non_nuke_thermal, dfGen[dfGen[!,:Zone].==z,:R_ID])))

	EP[:ePowerBalance] += ePowerBalanceThermNoCommit

	@expression(EP, ePowerBalanceNuclearNoCommit[t=1:T, z=1:Z],
		sum(EP[:vP][y,t] for y in intersect(nuclear, dfGen[dfGen[!,:Zone].==z,:R_ID])))

	EP[:ePowerBalance] += ePowerBalanceNuclearNoCommit
